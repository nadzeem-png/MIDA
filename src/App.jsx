...
const SHOW_SECTIONS = false; // hide left "Sections" list for a clean demo
  ...
  categories: [
    ...
    },
    {
      ...
      fields: [
        ...
        {
          key: "pillars",
          label: "Pillars: Economic Complexity, High Skilled Jobs, ESG, Domestic Linkage, Industrial Cluster",
          type: "longtext",
        },
    ...
    },
    {
      ...
      fields: [
        { key: "docs", label: "SSM Cert / License / Tax Letter / Land Title / Machines / R&D Plan", type: "file" },
...
function makeStyles(p) {
  return {
    shell: {
      minHeight: "100vh",
      width: "100%",              // full width; avoids 100vw overflow
      background: p.pageBg,
      color: p.text,
      overflowX: "hidden",
    },
    ...
    layout: {
      display: "grid",
      gridTemplateColumns: "260px minmax(0, 1fr)", // fluid content
      gap: 16,
      padding: 12,
      width: "100%",
      maxWidth: "100%",
    ...
    /* FORM GRID — responsive, no collisions */
    grid2: {
      display: "grid",
      columnGap: 16,
      rowGap: 14,
    ...
    input: {
      width: "100%",
      minWidth: 0, // critical in CSS grid
...
function CategoryCard({ moduleKey, category, color, state, setState, s }) {
  const twoCols = useTwoColumns(1200); // stack under 1200px
  return (
    <div style={s.card} id={`${moduleKey}.${category.key}`}>
      ...
      <div style={s.actions}>
        ...
        <button style={{ ...s.btnGhost, color: "#fff", background: color, borderColor: color }}>
          Submit Section
        </button>
      </div>
    </div>
    ...
    <div style={s.shell}>
      ...
      <div style={s.layout}>
        ...
        <aside style={s.sidebar}>
          ...
          <p style={{ fontSize: 12, color: palette.sub, marginTop: 0, marginBottom: 8 }}>
            Select a module
          </p>
          <div style={{ display: "grid", gap: 8 }}>
            {MODULES.map((m) => (
              <button
                ...
                style={s.moduleBtn(active === m.key, m.color)}
              >
          ...
          {SHOW_SECTIONS && current.categories?.length > 0 && (
            <div style={{ marginTop: 18 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 8 }}>Sections</div>
              <div style={{ display: "grid", gap: 6 }}>
                {current.categories.map((c) => (
                  <a key={c.key} href={`#${current.key}.${c.key}`} style={{ color: "#2563eb", textDecoration: "none", fontSize: 13 }}>
                    • {c.title}
                  </a>
                ))}
              </div>
