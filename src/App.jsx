...
const SHOW_SECTIONS = false; // keep hidden

/* ================= BLUEPRINTS ================= */
const NEW_INCENTIVES = { /* -- unchanged blueprint content trimmed for brevity -- */
  key: "new_incentives",
...
const DASHBOARD = {
  key: "dashboard",
  title: "Dashboard",
  color: "#e30613", // MIDA red
  categories: [], // not used (custom view)
};
...
function makeStyles(p) {
  return {
    shell: { minHeight: "100vh", background: p.pageBg, color: p.text, overflowX: "hidden" },
    topbar: {
    ...
    /* true full screen: no max widths anywhere */
    layout: { display: "grid", gridTemplateColumns: "220px minmax(0,1fr)", gap: 16, padding: 12, width: "100vw" },
    ...
    content: { display: "grid", gap: 16, width: "100%", maxWidth: "100%", paddingRight: 12 },
    ...
    grid3: { display: "grid", gap: 14, gridTemplateColumns: "repeat(3,minmax(0,1fr))" },
    ...
    barWrap: { height: 8, background: p.border, borderRadius: 999, overflow: "hidden" },
      ...
      <div style={s.grid3}>
        {kpis.map((k) => (
          <div key={k.label} style={s.card}>
            <div style={{ fontSize: 12, color: s.label.color }}>{k.label}</div>
          ...
          {progress.map((p) => (
            <div key={p.label}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                <span style={{ color: s.sub.color }}>{p.label}</span>
                <span>{p.value}%</span>
              </div>
            ...
            <div style={{ marginTop: 18 }}>
              ...
              <div style={{ display: "grid", gap: 6 }}>
                {current.categories.map((c) => (
                  <a key={c.key} href={`#${current.key}.${c.key}`} style={s.catLink}>• {c.title}</a>
                ))}
              </div>
            </div>
          )}
